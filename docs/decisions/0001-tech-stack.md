...
Roost v1 只支持 Windows，但必须同时满足 PRD 第 4、6、8、9、11、13、18、19 节的约束：小安装包、低常驻资源、透明置顶且仅内容区域接收点击、多 DPI、逐帧像素清晰、托盘/全局快捷键/全屏检测/单实例/自启、Windows 安全存储，以及将来的 OpenAI 兼容 HTTP 接口。
...
普通表单和设置页使用 Windows Forms；桌宠窗口使用无边框 Form，并以 Win32 PMv2 DPI awareness、窗口区域（`SetWindowRgn`）和必要的消息处理控制透明区域、命中区域和缩放变化。像素帧按物理像素整数倍用 nearest-neighbor 绘制。
...
| 13：常驻内存约 60–100 MB | 10 分钟 spike 平均 37.93 MB、峰值 39.80 MB，低于上限，给待办/UI/SQLite 留有余量。 | WebView2 至少包含浏览器/渲染进程，最容易逼近或超过 100 MB；必须另做 spike 才能确认。 | **最低且可控。** |
...
| 6.1：不同缩放 | PMv2 能看到显示器原始像素且不会被 Windows 位图缩放；需处理 `WM_DPICHANGED`。Microsoft 说明见 <https://learn.microsoft.com/en-us/windows/win32/hidpi/high-dpi-desktop-application-development-on-windows>。本机真实 DPI 覆盖不足，见下文。 | Tauri 暴露逻辑像素与 scale factor；WebView/CSS/Windows 合成链更长，像素清晰和命中换算风险较高。 | PMv2 + `WM_DPICHANGED` 全手工处理，控制力最高。 |
...
采用**方案 A：C# / .NET Framework 4.8 + Windows Forms + 少量 Win32 互操作**。
...
用户于 2026-09-20 确认采纳。真实 100% / 125% / 150% 缩放复测按用户决定延期到 M1 完成后执行；多显示器支持与验收要求从 PRD 删除。
...
- PRD 已按用户决定删除多显示器要求，并把真实 100% / 125% / 150% 缩放复测移到 M1 完成后的门禁；M0 以现有功能 spike、资源数据、技术栈确认和占位素材选定为完成依据。
